...
const AircraftAcquisition = async ({ params, searchParams }: PageProps) => {
  ...
  const technical = SERVICE_MENU_ITEMS.find(
    (item) => item.id === "technical-management"
  )!;
        ...
        >
          ...
          <div className="w-full lg:w-[62%] flex flex-col">
            ...
            <div className="w-full text-color-grey-3 text-xl font-normal leading-relaxed">
              {tl(`sections.section${i}.block.text`)}
            </div>
          </div>
        </div>
      }
    ...
    <>
      <div className="w-screen flex flex-col bg-stone-950 h-screen lg:px-16">
        <div className="__restricted-width relative top-0 left-0 h-[520px] lg:h-[1080px]">
          ...
          <NexeneImg
            src={"/images/aircraft-acquisition.svg"}
      ...
      <div className="__restricted-width lg:px-16">
        ...
        <ServiceNextButton
          url={`/services/${technical.id}`}
          text={tc(technical.name)}
        />
      </div>
    </>
  );
