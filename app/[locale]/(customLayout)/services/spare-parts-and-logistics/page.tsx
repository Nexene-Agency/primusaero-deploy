...
const SparePartsAndLogistics = async ({ params, searchParams }: PageProps) => {
  ...
  const privateAircraft = SERVICE_MENU_ITEMS.find(
    (item) => item.id === "private-aircraft-operation"
  )!;
    ...
    <>
      ...
      <div className="w-full flex flex-col bg-stone-950 h-screen lg:px-16">
        <div className="__restricted-width relative top-0 left-0 h-[520px] lg:h-[1080px]">
          ...
          <NexeneImg
            src={"/images/spare-parts-and-logistics.webp"}
      ...
      <div className="__restricted-width lg:px-16">
        ...
        <ServiceNextButton
          url={`/services/${privateAircraft.id}`}
          text={tc(privateAircraft.name)}
        />
      </div>
    </>
  );
