...
const AircraftCharter = async ({params, searchParams}: PageProps) => {
  ...
  const maintenance = SERVICE_MENU_ITEMS.find((item) => item.id === "maintenance")!;
  ...
  const renderSection = (i: number) => (
    <div key={`rs-${i}`}
      ...
      <div
        className="w-full lg:w-[62%] text-stone-950 text-xl font-normal leading-relaxed">{tl(`sections.section${i}.text`)}</div>
    </div>
  );
  ...
  return (
    <>
      ...
      <div className="__restricted-width">
        ...
        <ServiceNextButton url={`/services/${maintenance.id}`} text={tc(maintenance.name)}/>
      </div>
    </>
  );
};
