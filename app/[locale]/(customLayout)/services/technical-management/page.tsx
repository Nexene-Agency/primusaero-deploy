...
const TechnicalManagement = async ({ params, searchParams }: PageProps) => {
  ...
  const consulting = SERVICE_MENU_ITEMS.find(
    (item) => item.id === "consulting-services"
  )!;
      ...
      fallback={
        ...
        >
          ...
          <div className="w-full lg:w-[62%]">
            ...
            <div>{tl(`sections.section${i}.block.text`)}</div>
          </div>
        </div>
      }
    >
    ...
    <>
      <div className="w-screen flex flex-col bg-stone-950 h-screen lg:px-16">
        <div className="__restricted-width relative top-0 left-0 h-[520px] lg:h-[1080px]">
          ...
          <NexeneImg
            src={"/images/technical-management.webp"}
            className="block absolute max-h-screen object-center mx-auto mt-64 lg:mt-20 z-10"
          />
        </div>
      </div>

      <div className="__restricted-width">
        <div className="flex flex-col my-36 lg:my-72 lg:mx-16 items-center gap-16">
          {renderSection(0)}
          <div className="w-full lg:justify-between items-start flex-col lg:flex-row gap-6 lg:gap-0 inline-flex">
            ...
            <div className="w-full lg:w-[62%] pr-16">
        ...
        <ServiceNextButton
          url={`/services/${consulting.id}`}
          text={tc(consulting.name)}
        />
      </div>
    </>
  );
