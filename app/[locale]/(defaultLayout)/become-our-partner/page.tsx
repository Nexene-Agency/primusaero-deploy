...
import Block3 from "@app/components/home-page/block-3";
  ...
  const renderSection = (i: number) => (
    <div key={`rs-${i}`}
      ...
      <div className="w-full lg:w-[62%]">
        <div
          className="w-full text-stone-950 text-lg lg:text-xl font-normal leading-relaxed mb-8">{tl(`sections.section${i}.block.title`)}</div>
        <div
  ...
  return (
    <>
      ...
      <div className="__restricted-width __video-placeholder">
        <img src="/images/smaller_placeholder_image-transparent.webp" alt="video placeholder"/>
      </div>
      <div className="__restricted-width">
        <div className="flex flex-col my-36 lg:my-72 mx-6 lg:mx-16 items-center gap-16">
          {Array.from({length: 4}).map((_, i) => renderSection(i))}
        </div>
      </div>
      <div className="__restricted-width __video-placeholder">
        <img src="/images/smaller_placeholder_image-transparent.webp" alt="video placeholder"/>
