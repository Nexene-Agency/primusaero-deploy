...
const testimonialFallback = (testimonialCss: string, authorCss: string, text: string, author: string) => (
  <>
    <div
...
const BlockOne = (props: any) => {
  ...
  return (
    <div className="flex flex-col items-center mt-[108px] bg-stone-950">
      {/*--- Hero Section ---*/}
      <div className="w-[1536px] h-[764px] flex flex-col items-center z-101">
        {/*--- Headline ---*/}
        <div className="w-full h-[620px] absolute">
          <div className="w-full text-center mt-12">
            <span
              className="text-white text-5xl sm:text-7xl lg:text-9xl xl:text-[13.5rem] font-bold font-muller uppercase">
              PRIMUS
            </span>
            <span
              className="text-white text-5xl sm:text-7xl lg:text-9xl xl:text-[13.5rem] font-light font-muller uppercase">
              AERO
            </span>
          </div>
        </div>
        ...
        <div className={`hidden lg:flex flex-col w-full p-16 gap-6`}>
          <Suspense
            fallback={testimonialFallback("text-stone-950 leading-7 uppercase text-white self-stretch font-medium font-muller text-3xl",
              "text-neutral-500 text-xl", props.testimonial?.data?.text, props.testimonial?.data?.author)}>
            <TestimonialSlideshow timeout={5000}
                                  textCss="text-stone-950 leading-7 uppercase text-white self-stretch font-medium font-muller text-3xl"
      ...
      {/*--- Services ---*/}
      <div
        className="w-full h-14 px-16 bg-white  border-t border-b border-stone-950 backdrop-blur-2xl justify-center items-center inline-flex">
      ...
      <div className={`flex lg:hidden flex-col w-full px-6 py-36 bg-white gap-8 border-b border-stone-950`}>
        <Suspense
          fallback={testimonialFallback("leading-7 text-stone-950 self-stretch font-normal text-xl",
            "text-neutral-500 text-xl", props.testimonial?.data?.text, props.testimonial?.data?.author)}>
          <TestimonialSlideshow timeout={5000}
